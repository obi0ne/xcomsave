...
namespace xcom
{
	...
	xcom_version peek_header_save_game_version(xcom_io& r)
	{
	...
	actor_table read_actor_table_xcom1(xcom_io& r, xcom_version ver)
	{
		actor_table actors;
		...
		if (xcom_version::enemy_unknown != ver)
		{
			assert(actor_count % 2 == 0);
		}

		const int increment = (xcom_version::enemy_unknown == ver) ? 1 : 2;
	...
	actor_table_xcom2 read_actor_table_xcom2(xcom_io& r, xcom_version ver)
	{
		...
		{
			xcom2_actor_data data;

			data.serialized_idx = i;
			data.object_name = r.read_string();
			data.parent_id = r.read_int();
			dummy = r.read_int();
			data.object_size = r.read_int();
			dummy = r.read_int();
			actors.push_back( std::move(data) );
		}
	...
	checkpoint_chunk_table read_checkpoint_chunk_table(xcom_io& r, xcom_version version)
	{
		...
		do {
			...
			chunk.actors = read_actor_table_xcom1(r, version);
	...
	void dump_data_to_file(buffer<unsigned char>& buf, std::string output_file_name)
	{
		FILE* fp = fopen(output_file_name.c_str(), "wb");
		fwrite(buf.buf.get(), 1, buf.length, fp);
		fclose(fp);
	}



	std::variant<saved_game, saved_game_xcom2> read_xcom_save(buffer<unsigned char>&& b)
	{
		std::variant<saved_game, saved_game_xcom2> save;
		uint32_t header_size = 0;


		xcom_io rdr{ std::move(b) };

		auto game_version = peek_header_save_game_version(rdr);

		if (!supported_version(game_version))
		{
			throw error::unsupported_version(game_version);
		}
		...
		{
			save = saved_game{};
			auto& mySave = std::get<saved_game>(save);
			mySave.hdr = decode_xcom1_header(rdr, game_version);

			if (mySave.hdr.tactical_save)
			{
			...
			mySave.actors = read_actor_table_xcom1(uncompressed, game_version);
		...
		case xcom_version::xcom2_war_of_chosen:
		{
			save = saved_game_xcom2{};
			auto& mySave = std::get<saved_game_xcom2>(save);
			mySave.hdr = decode_xcom2_header(rdr, game_version);
			...
			header_size = mySave.hdr.header_size;
			buffer<unsigned char> uncompressed_buf = decompress(rdr, game_version, header_size);
